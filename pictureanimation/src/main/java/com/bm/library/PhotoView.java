...
//            mScaleCenter.set(detector.getFocusX(), detector.getFocusY());
    ...
    private void getLocation(int[] position) {
        ...
        while (viewParent instanceof PhotoView) {
            final PhotoView photoView = (PhotoView) viewParent;

            if (photoView.getId() == android.R.id.content) return;

            position[0] -= photoView.getScrollX();
            position[1] -= photoView.getScrollY();

            position[0] += photoView.getLeft();
            position[1] += photoView.getTop();

            viewParent = photoView.getParent();
        }
