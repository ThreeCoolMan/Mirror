...
public class TopicsShareActivity extends BaseActivity implements OkHttpNetHelperListener<TopicsShareBeans>, UrlListener {
    ...
    private ImageView backgroundIv, shareIv, closeIv;//背景 iv
    ...
    protected void initView() {
        ...
        closeIv = bindView(R.id.activity_topicsShare_iv_close);
...
// 分享时Notification的图标和文字  2.5.9以后的版本不调用此方法
        ...
        oks.setTitle(beans.getData().getList().get(listPosition).getStory_title());
        // titleUrl是标题的网络链接，仅在人人网和QQ空间使用
        oks.setTitleUrl(SHARE_URL);
        // text是分享文本，所有平台都需要这个字段
        oks.setText(beans.getData().getList().get(listPosition).getStory_des());
        // imagePath是图片的本地路径，Linked-In以外的平台都支持此参数
        //oks.setImagePath("/sdcard/test.jpg");//确保SDcard下面存在此张图片
        // url仅在微信（包括好友和朋友圈）中使用
        oks.setUrl(SHARE_URL);
        // comment是我对这条分享的评论，仅在人人网和QQ空间使用
        oks.setComment(beans.getData().getList().get(listPosition).getStory_des());
